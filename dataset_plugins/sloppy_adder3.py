...
def _make_sloppy_adder_test_case(left: int, center: int, right: int) -> Tuple[IOList, IOList]:
    ...
    inputs: IOList = [l1, l0, c1, c0, r1, r0]
...
def _build_from_config(cfg: Dict[str, Any]) -> DatasetResult:
    num_inputs = int(cfg.get("num_inputs", 6))
    ...
    center_range = int(sloppy_adder_cfg.get("center_range", 3))
    ...
    for left in range(left_range):
        for center in range(center_range):
            for right in range(right_range):
                ins, outs = _make_sloppy_adder_test_case(left, center, right)
                examples.append({"inputs": ins, "outputs": outs})

    return examples, num_inputs, num_outputs


register_plugin("sloppy-adder3", _build_from_config)
